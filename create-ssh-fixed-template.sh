#!/bin/bash
# SSH 패스워드 인증이 확실히 작동하는 템플릿 생성

VM_ID=9008
VM_NAME="ubuntu-22-04-5-ssh-fixed"
STORAGE="local-lvm"

echo "=== SSH 패스워드 인증 확실히 작동하는 300GB 템플릿 생성 ==="
...
# 6. SSH 확실히 작동하는 cloud-config 생성
echo "6. SSH 확실히 작동하는 설정 스크립트 생성 중..."
sshpass -p 'cloud1234' ssh -o StrictHostKeyChecking=no root@192.168.135.10 \
'mkdir -p /var/lib/vz/snippets'

sshpass -p 'cloud1234' ssh -o StrictHostKeyChecking=no root@192.168.135.10 \
'cat > /var/lib/vz/snippets/ssh-fixed-setup.yml << "EOF"
#cloud-config
...
# SSH 설정을 메인 파일에서 직접 수정
runcmd:
  # Guest Agent 먼저 시작
  - systemctl enable qemu-guest-agent
  - systemctl start qemu-guest-agent
  
  # SSH 설정 파일 직접 수정 (확실한 방법)
  ...
  - sshd -T | grep passwordauth > /tmp/sshd-config-check
...
sshpass -p 'cloud1234' ssh -o StrictHostKeyChecking=no root@192.168.135.10 \
"qm set $VM_ID --cicustom user=local:snippets/ssh-fixed-setup.yml"

# 8. VM 시작 및 설정 적용
echo "8. VM 시작하여 SSH 수정 설정 적용 중..."
sshpass -p 'cloud1234' ssh -o StrictHostKeyChecking=no root@192.168.135.10 \
"qm start $VM_ID"

# 9. VM이 자동으로 종료될 때까지 대기
echo "9. SSH 설정 적용 완료 대기 중... (최대 5분)"
timeout=300
...
echo ""
echo "=== SSH 패스워드 인증 확실히 작동하는 템플릿 생성 완료! ==="
echo "템플릿 ID: $VM_ID"
echo "템플릿 이름: $VM_NAME"
echo ""
echo "설정된 SSH 인증:"
echo "  - PasswordAuthentication yes (메인 설정파일에 직접 적용)"
